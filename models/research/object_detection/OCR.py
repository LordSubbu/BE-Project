...
image = cv.imread('C:/Users/Navin Subbu/Documents/BEEP/Project/models/research/object_detection/images/test/IMG20200213094926.jpg',cv.IMREAD_COLOR)
...
# inverted = cv.adaptiveThreshold(invert,255,cv.ADAPTIVE_THRESH_GAUSSIAN_C,\
#                     cv.THRESH_BINARY,11,2)
# cv.imshow('inverted', inverted)

# ret,inverted = cv.threshold(invert,127,255,cv.THRESH_TOZERO_INV)
# cv.imshow('inverted', inverted)
"""kernel = np.ones((1, 1), np.uint8)
invert = cv.dilate(Cropped, kernel, iterations=1)
invert = cv.erode(invert, kernel, iterations=1)
cv.imshow('Dilated-Erosion', invert)"""
